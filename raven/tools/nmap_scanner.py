...
class NmapScanner:
    ...
    def scan_host(
        self, host: str, ports: str = "1-1024", arguments: str = "-sV -sC"
    ) -> NmapScanResult:
        """Scan a single host"""
        ...
        try:
            result = self.nm.scan(
                host, ports, arguments=arguments, timeout=self.timeout
            )
    ...
    def scan_network(
        self, network: str, ports: str = "1-1024", arguments: str = "-sV"
    ) -> NmapScanResult:
        ...
        try:
            result = self.nm.scan(
                network, ports, arguments=arguments, timeout=self.timeout
            )
            execution_time = time.time() - start_time
    ...
    def scan_ports(
        self, host: str, ports: List[int], arguments: str = "-sV"
    ) -> NmapScanResult:
