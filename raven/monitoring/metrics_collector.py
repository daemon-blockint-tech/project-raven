...
class MetricsCollector:
    ...
    def _init_prometheus_metrics(self):
        ...
        self.threats_blocked = Counter(
            'raven_threats_blocked_total',
            'Total number of threats blocked',
            ['method']
        )
        
        self.detection_time = Histogram(
            'raven_detection_duration_seconds',
            'Time taken to detect threats'
        )
        
        self.active_hypotheses = Gauge(
            'raven_active_hypotheses',
            'Number of active threat hunting hypotheses'
        )
    ...
    def record_threat_detected(self, severity: str, threat_type: str):
        ...
        self._store_metric("threats_detected", 1.0, {"severity": severity, "type": threat_type})
    ...
    def get_metrics(self, metric_name: str, since: Optional[float] = None) -> List[Metric]:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            "threats_detected": {
                "total": sum(m.value for m in self.metrics_history.get("threats_detected", [])),
                "by_severity": self._aggregate_by_label("threats_detected", "severity"),
                "by_type": self._aggregate_by_label("threats_detected", "type")
            },
            "threats_blocked": {
                "total": sum(m.value for m in self.metrics_history.get("threats_blocked", [])),
