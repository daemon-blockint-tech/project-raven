...
class ResponseOrchestrator:
    ...
    def _generate_response_actions(self, threat: Any, threat_type: str) -> List[Dict[str, Any]]:
        """Generate appropriate response actions"""
        actions = []
        
        if threat_type == "zero_day":
            actions.extend([
                {"type": "containment", "action": "isolate_host", "priority": "critical"},
                {"type": "containment", "action": "block_ip", "priority": "high"},
                {"type": "remediation", "action": "harden_configuration", "priority": "medium"},
                {"type": "remediation", "action": "rotate_credentials", "priority": "medium"}
            ])
        elif threat_type == "lateral_movement":
            actions.extend([
                {"type": "containment", "action": "isolate_host", "priority": "critical"},
                {"type": "containment", "action": "disable_account", "priority": "high"},
                {"type": "remediation", "action": "update_firewall_rules", "priority": "medium"}
            ])
        elif threat_type == "data_exfiltration":
            actions.extend([
                {"type": "containment", "action": "block_ip", "priority": "critical"},
                {"type": "containment", "action": "terminate_process", "priority": "high"},
                {"type": "remediation", "action": "rotate_credentials", "priority": "medium"}
            ])
        else:
            # Default response
            actions.extend([
                {"type": "containment", "action": "isolate_host", "priority": "high"},
                {"type": "remediation", "action": "harden_configuration", "priority": "medium"}
            ])
    ...
    def execute_plan(self, plan: ResponsePlan) -> ResponseExecution:
        ...
        for action in plan.execution_order:
            ...
            if not result.success:
                ...
                if any(a.get("priority") == "critical" for a in plan.actions if a["action"] == action):
                    break
