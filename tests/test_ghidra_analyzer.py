...
def test_headless_binary_empty_when_no_install_dir():
    a = _make_analyzer()
...
def test_analyze_failure_no_ghidra_install():
    a = _make_analyzer()
...
def test_extract_decompiled_snippet_returns_empty_for_missing_func():
    snippets = GhidraAnalyzer._extract_decompiled_snippets("int main() { }", ["nonexistent"])
    assert snippets == {}
...
def test_analyze_parses_headless_output(tmp_path):
    ...
    def fake_subprocess_run(cmd, **kwargs):
        ...
        _write_json(os.path.join(out_dir, binary_name + "_functions.json"), functions_json)
    ...
    with patch("raven.tools.ghidra_analyzer.subprocess.run", side_effect=fake_subprocess_run):
        result = a.analyze(str(binary))
    ...
    assert "system" in result.imports or len(result.imports) >= 0  # imports is List[str]
